        ...
        <TabsContent value="mcp">
          <Card>
            <CardHeader className="flex flex-row items-start justify-between gap-4">
              ...
              <Dialog open={dialog === "mcp"} onOpenChange={(open) => setDialog(open ? "mcp" : null)}>
                <DialogTrigger asChild>
                  <Button>新增 MCP</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                  ...
                  <McpServerForm onSuccess={() => setDialog(null)} />
                </DialogContent>
              </Dialog>
            </CardHeader>
        ...
        <TabsContent value="chat-models">
          <Card>
            <CardHeader className="flex flex-row items-start justify-between gap-4">
              ...
              <Dialog
                open={dialog === "chat-models"}
                onOpenChange={(open) => setDialog(open ? "chat-models" : null)}
              >
                <DialogTrigger asChild>
                  <Button>新增 Chat Model</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>新增 Chat Model</DialogTitle>
                    <DialogDescription>第一版先覆盖 OpenAI Compatible / DashScope 一类配置。</DialogDescription>
                  </DialogHeader>
                  <ChatModelForm onSuccess={() => setDialog(null)} />
                </DialogContent>
              </Dialog>
            </CardHeader>
            <CardContent className="space-y-3">
        ...
        <TabsContent value="embedding-models">
          <Card>
            <CardHeader className="flex flex-row items-start justify-between gap-4">
              ...
              <Dialog
                open={dialog === "embedding-models"}
                onOpenChange={(open) => setDialog(open ? "embedding-models" : null)}
              >
                <DialogTrigger asChild>
                  <Button>新增 Embedding Model</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>新增 Embedding Model</DialogTitle>
                    <DialogDescription>支持 OpenAI Compatible 或本地模型两类接入方式。</DialogDescription>
                  </DialogHeader>
                  <EmbeddingModelForm onSuccess={() => setDialog(null)} />
                </DialogContent>
              </Dialog>
            </CardHeader>
            <CardContent className="space-y-3">
        ...
        <TabsContent value="rerank-models">
          <Card>
            <CardHeader className="flex flex-row items-start justify-between gap-4">
              ...
              <Dialog
                open={dialog === "rerank-models"}
                onOpenChange={(open) => setDialog(open ? "rerank-models" : null)}
              >
                <DialogTrigger asChild>
                  <Button>新增 Rerank Model</Button>
                </DialogTrigger>
                <DialogContent>
                  <DialogHeader>
                    <DialogTitle>新增 Rerank Model</DialogTitle>
                    <DialogDescription>用于提升 Retrieval 和 RAG Chat 的重排质量。</DialogDescription>
                  </DialogHeader>
                  <RerankModelForm onSuccess={() => setDialog(null)} />
                </DialogContent>
              </Dialog>
            </CardHeader>
            <CardContent className="space-y-3">
