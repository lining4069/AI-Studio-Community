...
class RerankModelRepository:
    ...
    async def get_by_id(self, model_id: str, user_id: int) -> Optional[RerankModel]:
        """Get Rerank model by ID"""
        stmt = select(RerankModel).where(
            RerankModel.id == model_id,
            RerankModel.user_id == user_id
        )
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()

    async def get_by_name(self, user_id: int, name: str) -> Optional[RerankModel]:
        """Get Rerank model by name for a user"""
        stmt = select(RerankModel).where(
            RerankModel.user_id == user_id,
            RerankModel.name == name
        )
        result = await self.db.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def list_by_user(
        ...
        # Count total
        count_stmt = select(func.count()).select_from(RerankModel).where(
            RerankModel.user_id == user_id
        )
        ...
        stmt = select(RerankModel).where(
            RerankModel.user_id == user_id
        ).order_by(RerankModel.created_at.desc()).offset(offset).limit(page_size)
        result = await self.db.execute(stmt)
    ...
    async def update(
        self, model: RerankModel, data: RerankModelUpdate
    ) -> RerankModel:
    ...
    async def get_default(self, user_id: int) -> Optional[RerankModel]:
